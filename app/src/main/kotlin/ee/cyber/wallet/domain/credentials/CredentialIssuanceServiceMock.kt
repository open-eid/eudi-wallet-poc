...
private const val MOCK_AGE_VERIFICATION_ISSUER_KEY_ID = "mdl_issuer"
...
class CredentialIssuanceServiceMock(
    ...
    fun issueMDocPid(pid: Credential.MdocPidCredential, keyAttestation: KeyAttestation): String {
        ...
        return mdoc.sign(
            ValidityInfo(signed, signed, Instant.fromEpochSeconds(1771581600L)),
            deviceKeyInfo,
    ...
    fun issueMDocMdl(mdl: Credential.MdocMdlCredential, keyAttestation: KeyAttestation): String {
        ...
        val signed = Clock.System.now()
        var validUntil = Instant.fromEpochSeconds(1771581600L)
        mdl.validUntil?.let {
            validUntil = it
        }
        return mdoc.sign(
    ...
    fun ageVerificationIssuerCryptoProvider(): SimpleCOSECryptoProvider {
    ...
    fun ageVerificationIssuerKeyPair(): ECKey {
    ...
    fun issueMDocAgeVerification(ageVerification: Credential.AgeVerificationCredential, keyAttestation: KeyAttestation): String {
        val deviceKeyInfo = DeviceKeyInfo(
            DataElement.fromCBOR(
                OneKey(keyAttestation.jwk.toECKey().toPublicKey(), null).AsCBOR().EncodeToBytes()
            )
        ...
        val mdoc = MDocBuilder(DocType.AGE_VERIFICATION.uri)
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "user_pseudonym", StringElement(ageVerification.userPseudonym))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "age_over_18", BooleanElement(ageVerification.ageOver18))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "issuance_date", FullDateElement(ageVerification.issuanceDate))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "expiry_date", FullDateElement(ageVerification.expiryDate))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "issuing_authority", StringElement(ageVerification.issuingAuthority))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "issuing_jurisdiction", StringElement(ageVerification.issuingJurisdiction))
            .addItemToSign(Namespace.EU_EUROPA_EC_EUDI_AGE_VERIFICATION_1.uri, "issuing_country", StringElement(ageVerification.issuingCountry))
        ...
        return mdoc.sign(
            ...
            MOCK_AGE_VERIFICATION_ISSUER_KEY_ID,
            status
        ).toCBORHex()
    }
}
